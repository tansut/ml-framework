...
class AnalyserJob:
    ...
    def create_neuralnet():
        ...
        self.neuralnet = self.neuralnet_class(train_x, train_y, self.args)
...
class NeuralNetAnalyser:
    ...
    def __init__(self, log_dir='./', labels=None, max_workers=None, executor=concurrent.futures.ProcessPoolExecutor, train_options=None):
        ...
        self.id = str(uuid.uuid4())
        NeuralNetAnalyser.analyser_list[self.id] = self
        self._predlock = Lock()
    ...
    def submit(self, neuralnet_class, train_set, test_sets, **kvargs):
        ...
        item = self.executor.submit(NeuralNetAnalyser.doit, job.id)
